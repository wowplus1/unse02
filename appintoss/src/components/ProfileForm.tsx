...
// 생년월일 입력 폼 → onSave(Profile) 콜백 (localStorage 저장은 App에서)
export default function ProfileForm({ initial, onSave, onCancel, compact }: {
  initial?: Profile | null; onSave: (p: Profile) => void; onCancel?: () => void; compact?: boolean;
}) {
  const p = initial;
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    onSave({
      ...
      hour: f.get("h") ? +(f.get("h") as string) : null,
      ...
      <form onSubmit={onSubmit}>
        ...
        <div className="row2 field">
          <div><label>태어난 시각</label>
            <select name="h" defaultValue={p?.hour ?? ""}>
        ...
        {onCancel && <button type="button" className="btn ghost mt" onClick={onCancel}>취소</button>}
      </form>
    </div>
  );
}
